...
/**
 * @class Ext.ux.data.ImprovedDWRProxy
 ...
 */
Ext.ux.data.ImprovedDWRProxy = function(config){
	Ext.apply(this, config); // necessary since the superclass doesn't call apply
	Ext.ux.data.ImprovedDWRProxy.superclass.constructor.call(this);
};

Ext.extend(Ext.ux.data.ImprovedDWRProxy, Ext.data.DataProxy, {
	...
	load: function(params, reader, loadCallback, scope, arg){
		...
		if (dataProxy.fireEvent("beforeload", dataProxy, params) !== false) {
			...
			if (this.dwrAdditional != null) {
	    		if (this.dwrAdditional instanceof Array) {
	    			for (var i = 0; i < this.dwrAdditional.length; i++) {
		    		    dwrFunctionArgs.push(this.dwrAdditional[i]);
		    		}
	    		} else {
	    			for (var key in this.dwrAdditional) {
	    				dwrFunctionArgs[key]=this.dwrAdditional[key]
	    			}
	    		}
		    }
			...
			dwrFunctionArgs.push({
				callback: function(response){
					...
					var records = reader.readRecords(json);
