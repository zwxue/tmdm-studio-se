...
public abstract class CoreOperation extends Operation {
    ...
    public boolean equals(Object obj) {
        if (! (obj instanceof CoreOperation)) return false;
        CoreOperation co = (CoreOperation)obj;

        if (co.isSymmetric() != this.isSymmetric()) return false;
    	if (! co.getSymbol().equals(this.getSymbol())) return false;
    	if (co.getPrecedence() != this.getPrecedence()) return false;

    	//predicates
    	if (this.args != null) {
    		if (co.args.length != this.args.length) return false;
    		for (int i = 0; i < this.args.length; i++) {
    			if (! co.args[i].equals(this.args[i])) return false;
            }
    	}
