...
public class RoutingEngineV2CtrlBean implements SessionBean, TimedObject {
    ...
    public RoutingRulePOJOPK[] route(ItemPOJOPK itemPOJOPK) throws XtentisException {
		...
		try {
			routingRuleCtrl = EnterpriseUtil.getRoutingRuleCtrlLocal();
		...
		try {
			routingOrderCtrl = EnterpriseUtil.getRoutingOrderV2CtrlLocal();
		} catch (NamingException e) {
		...
		try {
			itemCtrl = EnterpriseUtil.getItemCtrl2Local();
		} catch (NamingException e) {
		...
		List<String> serviceJndiList = EnterpriseUtil.getRuntimeServiceJndiList();
...
//					}
			...
			String userToken=EnterpriseUtil.getUsernameAndPasswordToken();
    ...
    private boolean ruleExpressionMatches(ItemPOJO itemPOJO, RoutingRuleExpressionPOJO exp) throws XtentisException{
    	...
    	try {
    		content = EnterpriseUtil.getFirstTextNode(itemPOJO.getProjection(),expXpath);
