...
public class User {
	...
	public String serialize() {
		...
		//if (roles!=null) 
		System.out.println("ROLE USER User.java"+roleNames.toString());
		{
			System.out.println("ROLE2");
	        Iterator iter = roleNames.iterator();
	        while(iter.hasNext()){
			//for (Iterator iter = roleNames.iterator(); iter.hasNext(); ) {
				user+="<role>"+((String) iter.next())+"</role>";
			}
		}
		user+=
			"    </roles>";
		user+=
			"    <properties>";
		if (dynamic!=null) {
			for (Iterator iter = dynamic.keySet().iterator(); iter.hasNext(); ) {
		...
		if (applications!=null) {
			for (Iterator iter = applications.keySet().iterator(); iter.hasNext(); ) {
				String key = (String) iter.next();
