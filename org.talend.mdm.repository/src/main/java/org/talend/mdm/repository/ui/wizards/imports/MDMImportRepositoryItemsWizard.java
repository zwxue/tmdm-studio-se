...
public class MDMImportRepositoryItemsWizard extends ImportItemsWizard {
    ...
    // private void importMatchRuleItems(Collection<ItemRecord> items) {
    // try {
    // ProxyRepositoryFactory factory = ProxyRepositoryFactory.getInstance();
    // List<IRepositoryViewObject> list = factory.getAll(IServerObjectRepositoryType.TYPE_MATCH_RULE, true, false);
    ...
    // }
    //
    // private void importMatchRuleItems( List<IRepositoryViewObject> newList,ItemRecord itemRecord, boolean overwrite)
    // {
    ...
    // for (IRepositoryViewObject current :newList) {
    ...
    // if ( current.getPath().equals(itemPath)) {
    // nameAvailable = false;
    // }
    // if (!nameAvailable) {
