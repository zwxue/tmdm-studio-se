...
/**
 * DOC hbhong class global comment. this class is compatible with
 * XpathSelectDialog, but works for Repository view
 */
public class XpathSelectDialog2 extends XpathSelectDialog {
	 ...
	 */
	public XpathSelectDialog2(Shell parentShell, String title,
			IWorkbenchPartSite site, boolean isMulti, String dataModelName,
			boolean isAbsolutePath) {
		super(parentShell, null, title, site, isMulti, dataModelName,
				isAbsolutePath);
	}
	...
	protected void changeDomTree(final TreeParent pObject, String filter) {
		...
		try {
			...
			if (null == xsd) {
				WSDataModelE wsDataModel = RepositoryQueryService
						.findDataModelByName(dataModelName);
				if (wsDataModel != null) {
					String schema = null;
