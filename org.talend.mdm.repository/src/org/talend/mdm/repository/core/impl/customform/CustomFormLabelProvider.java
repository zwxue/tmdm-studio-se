 ...
 * 
 */
public class CustomFormLabelProvider extends AbstractLabelProvider {

    private static final Image IMG = EclipseResourceManager.getImage(RepositoryPlugin.PLUGIN_ID, "icons/customform.png"); //$NON-NLS-1$;

    public String getCategoryLabel(ERepositoryObjectType type) {
        return "Custom Form"; //$NON-NLS-1$
    }
    ...
    protected String getServerObjectItemText(Item item) {
        WSCustomFormItem customFormItem = (WSCustomFormItem) item;
        WSCustomFormE form = customFormItem.getCustomForm();
        return form.getName();
    }
