 ...
 ****************************************************************************/
package com.amalto.workbench.widgets.xmlviewer.partition;
...
public class XMLDocumentPartitioner implements IDocumentPartitioner, IDocumentPartitionerExtension,
	 ...
	 */
	public static final String CONTENT_TYPES_CATEGORY = "__content_types_category"; //$NON-NLS-1$
	...
	public XMLDocumentPartitioner(IPartitionTokenScanner scanner, String legalContentTypes[]) {
		...
		positionCategory = "__content_types_category" + hashCode(); //$NON-NLS-1$
		...
		try {
			...
			if (first > 0) {
				...
				if (partition.includes(reparseStart)) {
					...
					contentType = "__dftl_partition_content_type"; //$NON-NLS-1$
				}
			}
	...
	public String getContentType(int offset) {
		...
		return p != null && p.includes(offset) ? p.getType() : "__dftl_partition_content_type"; //$NON-NLS-1$
	}
	...
	public ITypedRegion getPartition(int offset) {
		try {
			...
			if (category == null || category.length == 0) {
				return new TypedRegion(0, document.getLength(), "__dftl_partition_content_type"); //$NON-NLS-1$
			}
			int index = document.computeIndexInCategory(positionCategory, offset);
			if (index < category.length) {
				...
				if (index == 0) {
					return new TypedRegion(0, next.offset, "__dftl_partition_content_type"); //$NON-NLS-1$
				}
				...
				return new TypedRegion(endOffset, next.getOffset() - endOffset, "__dftl_partition_content_type"); //$NON-NLS-1$
			}
			...
			int endOffset = previous.getOffset() + previous.getLength();
			return new TypedRegion(endOffset, document.getLength() - endOffset, "__dftl_partition_content_type"); //$NON-NLS-1$
		} catch (BadPositionCategoryException _ex) {
		} catch (BadLocationException _ex) {
		}
		return new TypedRegion(0, document.getLength(), "__dftl_partition_content_type"); //$NON-NLS-1$
	...
	public ITypedRegion getPartition(int offset, boolean preferOpenPartitions) {
		...
		if (preferOpenPartitions && region.getOffset() == offset
				&& !region.getType().equals("__dftl_partition_content_type")) { //$NON-NLS-1$
			if (offset > 0) {
				region = getPartition(offset - 1);
				if (region.getType().equals("__dftl_partition_content_type")) { //$NON-NLS-1$
					return region;
				}
			}
			return new TypedRegion(offset, 0, "__dftl_partition_content_type"); //$NON-NLS-1$
		}
		...
		try {
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				if (includeZeroLengthPartitions && overlapsOrTouches(gap, offset, length) || gap.getLength() > 0
					...
					list.add(new TypedRegion(start, end - start, "__dftl_partition_content_type")); //$NON-NLS-1$
				}
			...
			if (list.isEmpty()) {
				list.add(new TypedRegion(offset, length, "__dftl_partition_content_type")); //$NON-NLS-1$
			}
		} catch (BadPositionCategoryException _ex) {
		}
