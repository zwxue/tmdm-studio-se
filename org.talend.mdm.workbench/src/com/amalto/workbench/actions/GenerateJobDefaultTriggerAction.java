...
public class GenerateJobDefaultTriggerAction extends Action {
    ...
    public void run() {
        ...
        try {
            ...
            WSRoutingRule routingRule = new WSRoutingRule((String) "CallJob_" + filename, "Trigger that calls the Talend Job: "//$NON-NLS-1$ //$NON-NLS-2$
                    + filename, false, "Update", "amalto/local/service/callJob", parameter, new WSRoutingRuleExpression[] {//$NON-NLS-1$ //$NON-NLS-2$
            ...
            TreeObject obj = new TreeObject(routingRule.getName(), xobject.getServerRoot(), TreeObject.ROUTING_RULE,
                    new WSRoutingRulePK((String) routingRule.getName()), routingRule // no storage to save space
            );
