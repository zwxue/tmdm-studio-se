...
public class GenerateJobDefaultTriggerAction extends Action {
    ...
    public void run() {
        ...
        try {
            ...
            WSRoutingRuleExpression express1 = new WSRoutingRuleExpression("C1", "Update/OperationType",//$NON-NLS-1$ //$NON-NLS-2$ 
                    WSRoutingRuleOperator.CONTAINS, "CREATE");//$NON-NLS-1$ 
            WSRoutingRuleExpression express2 = new WSRoutingRuleExpression("C2", "Update/OperationType",//$NON-NLS-1$ //$NON-NLS-2$
                    WSRoutingRuleOperator.CONTAINS, "UPDATE");//$NON-NLS-1$ 
            WSRoutingRuleExpression express3 = new WSRoutingRuleExpression("C3", "Update/OperationType",//$NON-NLS-1$ //$NON-NLS-2$
                    WSRoutingRuleOperator.CONTAINS, "DELETE");//$NON-NLS-1$ 
            List<WSRoutingRuleExpression> expressions = new ArrayList<WSRoutingRuleExpression>();
            expressions.add(express1);
            expressions.add(express2);
            expressions.add(express3);
            WSRoutingRule routingRule = new WSRoutingRule("CallJob_" + filename, "Trigger that calls the Talend Job: "//$NON-NLS-1$ //$NON-NLS-2$
                    + filename, false, "Update", "amalto/local/service/callJob", parameter, expressions, "C1 Or C2 Or C3", false);//$NON-NLS-1$

            Util.getPort(xobject).putRoutingRule(new WSPutRoutingRule(routingRule));
            // add the object to the tree
            TreeObject obj = new TreeObject(routingRule.getName(), xobject.getServerRoot(), TreeObject.ROUTING_RULE,
                    new WSRoutingRulePK(routingRule.getName()), routingRule // no storage to save space
            );
