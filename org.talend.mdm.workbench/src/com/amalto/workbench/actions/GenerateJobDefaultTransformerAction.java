...
public class GenerateJobDefaultTransformerAction extends Action {
    ...
    public void run() {
        ...
        JobProcesssOptionsDialog dialog = new JobProcesssOptionsDialog(server.getSite().getShell(), defaultExecutionType);
        ...
        List<WSTransformerProcessStep> steps = new ArrayList<WSTransformerProcessStep>();
        List<WSTransformerVariablesMapping> input;
        List<WSTransformerVariablesMapping> output;
        try {
            ...
            case CONTEXT_VARIABLE:
                ...
                input = getEmptyWSTransformerVariablesMapping();
                input.add(new WSTransformerVariablesMapping("_DEFAULT_", "xml", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                output = getEmptyWSTransformerVariablesMapping();
                output.add(new WSTransformerVariablesMapping("item_xml", "text", null));//$NON-NLS-1$ //$NON-NLS-2$ 

                steps.add(new WSTransformerProcessStep("amalto/local/transformer/plugin/xslt",//$NON-NLS-1$ 
                ...
                // step 2
                input = getEmptyWSTransformerVariablesMapping();
                input.add(new WSTransformerVariablesMapping("item_xml", "law_text", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                output = getEmptyWSTransformerVariablesMapping();
                output.add(new WSTransformerVariablesMapping("decode_xml", "codec_text", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                String parameter = "<parameters>\n" + "<method>DECODE</method>\n" + "<algorithm>XMLESCAPE</algorithm>\n"//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ 
                        + "</parameters>\n";//$NON-NLS-1$ 
                steps.add(new WSTransformerProcessStep("amalto/local/transformer/plugin/codec", "Escape the item XML",//$NON-NLS-1$ //$NON-NLS-2$ 
                        parameter, input, output, false));
                // Generate the codec step to escape the item xml
                // step 3
                input = getEmptyWSTransformerVariablesMapping();
                input.add(new WSTransformerVariablesMapping("decode_xml", "text", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                output = getEmptyWSTransformerVariablesMapping();
                output.add(new WSTransformerVariablesMapping("output", "result", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                parameter = "<configuration>\n" + "<url>" + url + "</url>\n" + "<contextParam>\n"//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ 
                        + "<name>xmlInput</name>\n" + "<value>{decode_xml}</value>\n" + "</contextParam>\n"//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ 
                        + "</configuration>\n";//$NON-NLS-1$ 
                steps.add(new WSTransformerProcessStep("amalto/local/transformer/plugin/callJob", "Invoke the job", parameter,//$NON-NLS-1$ //$NON-NLS-2$ 
                        input, output, false));
            ...
            case INTEGRATED:
                ...
                input = getEmptyWSTransformerVariablesMapping();
                input.add(new WSTransformerVariablesMapping("_DEFAULT_", "text", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                output = getEmptyWSTransformerVariablesMapping();
                output.add(new WSTransformerVariablesMapping("item_xml", "result", null));//$NON-NLS-1$ //$NON-NLS-2$ 
                parameter = "<configuration>\n" + "<url>" + url + "</url>\n" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ 
                        + "</configuration>\n"; //$NON-NLS-1$ 
                steps.add(new WSTransformerProcessStep("amalto/local/transformer/plugin/callJob",//$NON-NLS-1$ 
                        "Invoke the job", parameter, input, output, false)); //$NON-NLS-1$  
                break;
            default:
        ...
        try {
            ...
            TreeParent folder = xobject.findServerFolder(TreeObject.TRANSFORMER);
            folder.addChild(obj);
        } catch (Exception e) {
    ...
    private List<WSTransformerVariablesMapping> getEmptyWSTransformerVariablesMapping() {
        return new ArrayList<WSTransformerVariablesMapping>();
    }
}
