...
/**
 * TriggerContentTest is a test class to imitate the process to create the steps
 * of a Trigger.This class will imitate the way to create a Trigger like
 * DStarAgentCommissionChangeModified in DStar.
 * 
...
public class TriggerContentTest extends TalendSWTBotForMDM {
	...
	private String[] array = new String[] { "DataModel  [0...1]", "Concept",
			"OperationType" };
	...
	public void setServiceTest() {
		...
		bot.textWithLabel("Service Parameters").setText(
				"process=UpdateOperationalSystems");
		Assert.assertEquals("callprocess",
				bot.comboBoxWithLabel("Service JNDI Name").getText());
	}
	...
	public void setXpathTest() {
		...
		bot.comboBox().setSelection("UpdateReport");
		sleep();
		SWTBotTreeItem parent = bot.tree().getTreeItem("Update").expand()
				.getNode("Update").expand();
		parent.select(array);
		sleep();
		bot.button("Add").click();
		sleep();
		...
		bot.comboBox().setSelection("UpdateReport");
		sleep();
		parent = bot.tree().getTreeItem("Update").expand().getNode("Update")
				.expand().getNode("Item  [0...many]").expand().getNode(0)
				.expand();
